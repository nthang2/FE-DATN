...
export const apiUrl = {
  ...
  getLiquidationLeaderboard: ({ user, collateral, healthFactorThreshold, sortBy, reverse }: TLiquidationLeaderboardParams) => {
    ...
    return `${BACKEND_URL}/lending/liquidates/risk_loans?'${_user}${_collateral}${checkFinalParam}healthFactorThreshold=${healthFactorThreshold}&sortBy=${sortBy}&reverse=${reverse}`;
  },
};
